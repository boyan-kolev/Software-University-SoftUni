...
SELECT  f1.Id,
		f1.[Name],
		(CAST(f1.Size AS VARCHAR(100)) + 'KB') AS [Size]
FROM Files AS f1
...
BEGIN
	...
	DECLARE @countOfCommits INT = (SELECT COUNT(*) FROM Commits WHERE ContributorId = @userId)
...
CREATE PROC usp_FindByExtension(@extension VARCHAR(100))
AS
SELECT Id,
		[Name],
		(CAST(Size AS VARCHAR(MAX)) + 'KB') AS [Size]
FROM Files
